...
//External js pages
import ABI from './abi/EtherWallet.json';
  ...
  const renderPage = (key) => {
    switch (key) {
      case "0":
        return <Welcome />
      case "1":
        return <TransferTo currentAddress={currentAddress} isConnect={isConnect} contract={contract} accFunc={getAccBAl} />
      case "2":
        return <History contract={contract} web3={web3} allright={allright} isConnect={isConnect} cA={currentAddress} />
      case "3":
        return <DepositAndWithdraw currentAddress={currentAddress} isConnect={isConnect} contract={contract} accFunc={getAccBAl} />
      default:
        return <Welcome />
    }
  };
    ...
    <>
      <div className="myNavbar-container">
        <div className="myNavbar">
          <Container style={{ width: "0%", marginRight: "11%", cursor: "pointer" }}>
            <Row>
              <Col xs={2} md={2}>
                <Image onClick={handleClickOnImage} src={logo} rounded />
              </Col>
            </Row>
          </Container>
          <Nav variant="pills" activeKey={activeKey} onSelect={handleSelect} style={{ display: "flex", justifyContent: "space-around", maxWidth: "max-content" }}>
          <NavItem>
              <NavLink className={`mynav-link ${activeKey === "3" ? "active" : ""}`} eventKey="3">
                Deposit/Withdraw
              </NavLink>
            </NavItem>
            <NavItem>
        ...
        {isConnect ? <div className="addr-txt ">{trimAddr(currentAddress)}</div> : (<Button variant="primary" className="btn-connect" onClick={walletConnect}> Connect </Button>)}
        <div style={{position:"absolute",marginLeft:"60%",marginTop:"7%"}}>
        {isConnect?
        (
          (accBal==='')?(<Button className="accBal-btn" variant="primary" onClick={getAccBAl}>Get Balance</Button>): renderAccBal()
        ) 
        :(<></>)}
        </div>
      </div>
